...
public class BatchData {
  ...
  protected List<PoolBinary[]> binaryRet;
  ...
  public void init(TSDataType type) {
    ...
    switch (dataType) {
      ...
      case STRING:
        ...
        binaryRet.add(new PoolBinary[capacity]);
        break;
      case VECTOR:
   ...
   * @param v binary data.
   */
  public void putBinary(long t, PoolBinary v) {
    if (writeCurArrayIndex == capacity) {
      if (capacity >= CAPACITY_THRESHOLD) {
        timeRet.add(new long[capacity]);
        binaryRet.add(new PoolBinary[capacity]);
        ...
        long[] newTimeData = new long[newCapacity];
        PoolBinary[] newValueData = new PoolBinary[newCapacity];
  ...
  public void setBinary(PoolBinary v) {
  ...
  public PoolBinary getBinaryByIndex(int idx) {
  ...
  public void serializeData(DataOutputStream outputStream) throws IOException {
    switch (dataType) {
      ...
      case STRING:
        for (int i = 0; i < length(); i++) {
          outputStream.writeLong(getTimeByIndex(i));
          PoolBinary binary = getBinaryByIndex(i);
          ...
          for (TsPrimitiveType value : values) {
            if (value == null) {
              ...
              switch (value.getDataType()) {
                ...
                case BLOB:
                case STRING:
                  PoolBinary binary = value.getBinary();
                  outputStream.writeInt(binary.getLength());
                  outputStream.write(binary.getValues());
                  break;
                case INT64:
