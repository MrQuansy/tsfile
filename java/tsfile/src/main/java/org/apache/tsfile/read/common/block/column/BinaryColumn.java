...
public class BinaryColumn implements Column {
  ...
  private final PoolBinary[] values;

  private final long retainedSizeInBytes;

  public BinaryColumn(int initialCapacity) {
    this(0, 0, null, new PoolBinary[initialCapacity]);
  }

  public BinaryColumn(int positionCount, Optional<boolean[]> valueIsNull, PoolBinary[] values) {
    this(0, positionCount, valueIsNull.orElse(null), values);
  }

  BinaryColumn(int arrayOffset, int positionCount, boolean[] valueIsNull, PoolBinary[] values) {
    if (arrayOffset < 0) {
  ...
  @Override
  public PoolBinary getBinary(int position) {
    return values[position + arrayOffset];
  }

  @Override
  public PoolBinary[] getBinaries() {
    return values;
  }
  ...
  public Column getRegionCopy(int positionOffset, int length) {
    ...
    PoolBinary[] valuesCopy = Arrays.copyOfRange(values, from, to);
  ...
  public Column subColumnCopy(int fromIndex) {
    ...
    PoolBinary[] valuesCopy = Arrays.copyOfRange(values, from, positionCount);
